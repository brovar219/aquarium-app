...
- **Погода** (`weather_client`) — чисті функції геокодингу/запиту Open-Meteo
  по HTTPS (cert-bundle, без API-ключа). Фонова задача в `aquarium_app.cpp`
  оновлює стан; `DeviceService` лише зберігає результат і (за галочкою)
  застосовує до денного таргета.

### Фонові задачі (FreeRTOS)

| Задача | Період | Стек | Роль |
|--------|--------|------|------|
| `aq_tick` | 100 мс | 8192 | світло/сенсор/розклад, дренаж MQTT-черги |
| `aq_storm` | 8 мс (лише шторм) | 4096 | мерехтіння грози, єдиний письменник світла під час шторму |
| `aq_weather` | 20 хв (ретрай 60 с) | 8192 | HTTPS до Open-Meteo |
| DS18B20 | 5 с | — | фонове читання температури |

## Тести

- `test/host/` — хостові юніт-тести `ScheduleEngine` (CMake + ctest, без IDF).
- `web/ui-logic.test.mjs` — пермінатор чистої UI-логіки (Node).
- `.github/workflows/ci.yml` — збірка прошивки + обидва набори тестів.
